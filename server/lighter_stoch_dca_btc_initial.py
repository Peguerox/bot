"""
Real-money Lighter BTC bot -- "Stochastic5", INITIAL settings variant.

Run side by side with the "optimal" and "current" (Worker 3) variants for a real A/B/C comparison
at $100 each. Same fixed, bug-free engine as the other two (TP-direction fix, position-resync
guard, live bid/ask OCO check, no time limit, no DCA) -- only the tuning parameters differ.

Settings: entry AND reversal both at 20/80 (no split), TP=0.10% off avg entry, SL=0.10% off first
entry (fixed). No DCA -- single leg, full equity.
"""
import asyncio
import os
import time
...
async def main():
    print("Stochastic5 DCA bot (INITIAL settings) starting (BTC, real money, $100 seed)")
    while True:
        try:
            await tick()
        except Exception as e:
