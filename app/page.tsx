  ...
  return (
    <div className="bg-gray-900 rounded-xl p-5 space-y-5 flex flex-col">
      <div className="space-y-1.5">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <h2 className="text-white font-bold text-lg">SOL Jump Live</h2>
        ...
        <p className="text-gray-500 text-xs">Binance SOLUSDT jump-signal (≥0.02% in 2s) → Bitfinex tSOLUSD real buy · LIVE · REAL MONEY · $20 seed, compounds · long-only (no shorting on spot) · no TP · 0.1% trailing stop · NOT the Pure Trail strategy — converted from that to Jump Trail 2026-09-03</p>
      </div>
  ...
  return (
    <div className="bg-gray-900 rounded-xl p-5 space-y-5 flex flex-col">
      <div className="space-y-1.5">
        ...
        <p className="text-gray-500 text-xs">Binance SOLUSDT jump-signal (≥0.02% in 2s) → Bitfinex tSOLUSD long entry, long-only (no shorting on spot) · PAPER · $100 seed, compounds · 0.05% trailing stop (head-to-head vs SOL Jump Live's 0.1%) · spread updated to 0.0267% (old + observed live extra cost)</p>
      </div>
...
export default function Dashboard() {
  ...
  async function handleSolTrailContinuousClearHistory() {
    if (!confirm("Delete all SOL Jump Live trade history and run logs?")) return;
  ...
  async function handleShowSummary() {
    ...
    const bots = [
      ...
      { name: "SOL Jump Live", badge: "LIVE", state: solTrailContinuousState, runsTable: "sol_trail_continuous_runs", pnlField: "realized_pnl_usd", initial: 20, unit: "$", venue: "bitfinex" as const, symbol: "tSOLUSD" },
