// Z-score strategy, 1-min high/low execution, NO max-hold timeout — trade stays open until it
// actually hits TP (+0.8%) or SL (-0.3%), only two possible outcomes. All 15 original coins,
// 1 week, Binance Global, 0% fee.
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number) {
  ...
  while (i < candles5.length) {
    ...
    if (zscores[i] <= Z_ENTRY) {
      ...
      if (exitPrice === null) {
        // ran out of data before resolving — mark as still open at last known price, don't count as a completed trade
        const lastPrice = candles1.length ? candles1[candles1.length - 1].c : entryPrice;
        usd = qty * lastPrice;
        qty = 0;
        stillOpen++;
        break; // no more data to continue simulating
      }
...
(async () => {
  ...
  console.log(`\nZ-score, 1-min HL execution, NO timeout (TP/SL only) · 1 week · Binance Global · 0% fee\n`);
  for (const r of results) {
    console.log(`${r.coin.padEnd(6)}${(r.ret >= 0 ? "+" : "") + r.ret.toFixed(1).padStart(8)}%   $${r.finalVal.toFixed(2).padStart(8)}   trades=${String(r.trades).padStart(3)}   WR=${r.wr.toFixed(1).padStart(5)}%   maxDD=${r.maxDD.toFixed(1)}%   TP=${r.tpHits} SL=${r.slHits}${r.stillOpen ? ` (${r.stillOpen} still open)` : ""}`);
  }
})();
