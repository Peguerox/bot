// Same as zscore-1m-hl-1wk-global.ts but 3-week window, ETC/BCH/LINK/BNB only (SOL/DOT dropped).
...
(async () => {
  const now = Date.now();
  const windowStart = now - 21 * 24 * 60 * 60 * 1000; // 3 weeks
  const candleFetchStart = windowStart - (ZSCORE_WINDOW + 5) * CANDLE_MS;

  const coins = ["ETC", "BCH", "LINK", "BNB"];
  const results: any[] = [];
  ...
  console.log(`\nZ-score, 1-min high/low execution · 3 weeks · Binance Global · 0% fee\n`);
