// Take the lowest LOW of the last 12 five-minute candles as a target buy price, place a
// resting limit buy there, and wait (continuously, checked every 1 minute) until price
// actually touches it. Once filled: TP=0.3%/SL=0.1%. Target recomputes as new 5m candles
// close while waiting for a fill. SOLFDUSD, 3 months.
import dotenv from "dotenv";
dotenv.config({ path: ".env.local" });

const BASE = "https://data-api.binance.vision/api/v3";
const ALLOCATION_USD = 50;
const LOW_LOOKBACK = 12;
const TP_PCT = 0.3;
const SL_PCT = 0.1;
const CANDLE_MS = 5 * 60 * 1000;
const MIN_MS = 60 * 1000;

type C5 = { t: number; l: number };
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number) {
  ...
  let c5Idx = 0; // pointer to most recently closed 5m candle
  ...
  for (let i = start1Idx; i < candles1.length; i++) {
    ...
    // advance c5Idx to the last 5m candle that has fully closed before this 1m candle
    while (c5Idx + 1 < candles5.length && candles5[c5Idx + 1].t + CANDLE_MS <= t) c5Idx++;

    if (!inTrade) {
      if (c5Idx >= LOW_LOOKBACK) {
        let target = Infinity;
        for (let k = c5Idx - LOW_LOOKBACK; k < c5Idx; k++) target = Math.min(target, candles5[k].l);
        if (candles1[i].l <= target) {
          // filled at target price
          entryPrice = target;
          ...
          inTrade = true;
          fills++;
          continue; // don't check exit on the same candle as entry
        }
      }
    } else {
...
(async () => {
  ...
  console.log(`\nSOLFDUSD buy-at-12-candle-low (limit) · 3mo · TP=0.3%/SL=0.1%\n`);
