// Compare signal thresholds for Pure Lag BTC bot
...
function runSim(aligned: { high: number; low: number; close: number }[], usClose: number[], glClose: number[], thresh: number) {
  ...
  for (let i = 1; i < aligned.length; i++) {
    ...
    if (spread >= thresh && glRet > 0 && usRet < glRet) pending = close * (1 + ENTRY_SLIP);
  }
...
(async () => {
  ...
  console.log(`TP=${TP_PCT*100}%  SL=${SL_PCT*100}%  MAX_HOLD=${MAX_HOLD}  $${ALLOCATION} compounding  direction filter ON\n`);
  console.log("  Threshold   Trades  /day   Fill%    WR%     PF      PnL$      Ret%   MaxDD%");
  console.log("  " + "─".repeat(74));

  for (const thresh of [0.00005, 0.0001, 0.0002, 0.0003, 0.0005, 0.001, 0.002, 0.003]) {
    const r = runSim(aligned, usClose, glClose, thresh);
    const pf = r.pf === Infinity ? "   ∞" : r.pf.toFixed(2);
    console.log(
      `  ${(thresh*100).toFixed(4)}%`.padEnd(13) +
      `${r.trades}`.padStart(7) +
