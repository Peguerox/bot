// New angle for SOLBTC: filter entries by BTC's OWN trend (BTCUSDT vs its 90d/200d MA),
// not the SOL/BTC ratio (which is what every prior filter tested). Idea: SOLBTC's losing
// years (2022, 2026) coincide with BTC itself being in a broader bear market — maybe
// avoiding entries while BTC is below its own long-term trend helps. Layered on top of
// the flat trailing stop (arm@6%, trail@7.5pp). BTC-denominated, continuous ~5yr.
...
// simple MA on daily BTCUSDT closes
function btcSmaAt(btcDaily: C[], smaArr: number[], t: number): number {
  let lo = 0, hi = btcDaily.length - 1, idx = -1;
  while (lo <= hi) { const mid = (lo + hi) >> 1; if (btcDaily[mid].t <= t) { idx = mid; lo = mid + 1; } else hi = mid - 1; }
  return idx >= 0 ? smaArr[idx] : NaN;
}
...
function runSim(c15: C[], c12h: C[], btcUsd: C[], btcDaily: C[], btcSma: number[], useRegimeFilter: boolean, label: string) {
  ...
  for (let i = 1; i < c15.length; i++) {
    ...
    if (mode === "BTC" && armedForSol && bullish && sloping) {
      let isBlocked = false;
      if (useRegimeFilter) {
        const sma = btcSmaAt(btcDaily, btcSma, t);
        if (!isNaN(sma) && usdPx < sma) isBlocked = true;
      }
  ...
  console.log(`${label.padEnd(36)}${(btcAccumRet>=0?"+":"")+btcAccumRet.toFixed(1).padStart(8)}%   $${finalUsd.toFixed(2).padStart(8)}   trades=${trades}   maxDD=${maxDD.toFixed(1)}%   blocked=${blocked}`);
}
...
(async () => {
  ...
  process.stdout.write(`Fetching BTCUSDT 1d... `); const btcDaily = await fetchKlines("BTCUSDT", "1d", start, now); console.log(`${btcDaily.length}`);

  const sma90 = calcSMA(btcDaily, 90);
  const sma200 = calcSMA(btcDaily, Math.min(200, btcDaily.length - 1));

  console.log(`\nSOLBTC · continuous ~5yr · BTC-regime entry filter (on top of arm6/trail7.5)\n`);

  runSim(c15, c12h, btcUsd, btcDaily, sma90, false, `[BASELINE] no BTC regime filter`);
  runSim(c15, c12h, btcUsd, btcDaily, sma90, true, `+ block if BTC < its 90d SMA`);
  runSim(c15, c12h, btcUsd, btcDaily, sma200, true, `+ block if BTC < its 200d SMA`);
})();
