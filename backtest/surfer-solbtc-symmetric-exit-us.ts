// New angle for SOLBTC: symmetric entry/exit rigor. Entry requires RSI arm + EMA bullish +
// EMA7 sloping UP (3 conditions). Exit currently only requires RSI arm + EMA bearish (2
// conditions, no slope check) — treating "switch to BTC" as a lower bar than "switch to
// SOL." This tests requiring EMA7 sloping DOWN too on exit, same rigor both directions —
// since the goal is coin accumulation regardless of which side you're holding, both switches
// should be held to the same standard. Layered on top of trailing stop (arm@6%, trail@7.5pp).
...
function runSim(c15: C[], c12h: C[], btcUsd: C[], requireSlopeDown: boolean, label: string) {
  ...
  function getTrend(t: number, livePrice: number) {
    ...
    if (idx < 0) return { bullish: false, slopingUp: false, slopingDown: false };
    const { fast, prevFast, slow, close } = trend12h[idx];
    if (isNaN(fast) || isNaN(slow)) return { bullish: false, slopingUp: false, slopingDown: false };
    const delta = livePrice - close;
    const liveFast = fast + delta / MA_FAST;
    const liveSlow = slow + delta / MA_SLOW;
    return {
      bullish: liveFast > liveSlow,
      slopingUp: !isNaN(prevFast) && liveFast > prevFast,
      slopingDown: !isNaN(prevFast) && liveFast < prevFast,
    };
  }
  ...
  for (let i = 1; i < c15.length; i++) {
    ...
    const { bullish, slopingUp, slopingDown } = getTrend(t, price);

    if (mode === "BTC" && armedForSol && bullish && slopingUp) {
    ...
    if (mode === "SOL") {
      ...
      else if (armedForBtc && !bullish && (!requireSlopeDown || slopingDown)) closeNow = true;
    }
    if (closeNow) {
      btc = solQty * price;
      trades++;
  ...
  console.log(`${label.padEnd(42)}${(btcAccumRet>=0?"+":"")+btcAccumRet.toFixed(1).padStart(8)}%   $${finalUsd.toFixed(2).padStart(8)}   trades=${trades}   maxDD=${maxDD.toFixed(1)}%`);
}
...
(async () => {
  ...
  console.log(`\nSOLBTC · continuous ~5yr · symmetric exit test (arm6/trail7.5)\n`);

  runSim(c15, c12h, btcUsd, false, `[BASELINE] exit: no slope-down requirement`);
  runSim(c15, c12h, btcUsd, true,  `+ require EMA7 sloping DOWN on exit too`);
})();
