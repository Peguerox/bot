// Compares BCHFDUSD vs BCHUSDT on the same z-score strategy (1-min HL execution, no timeout,
// TP=0.8%/SL=0.3%), 1 month, to check if the actual 0%-fee pair (FDUSD) holds up the same as
// the USDT pair we validated everything on.
...
(async () => {
  ...
  for (const symbol of ["BCHFDUSD", "BCHUSDT"]) {
    ...
    console.log(`${symbol.padEnd(10)}${(r.ret>=0?"+":"")+r.ret.toFixed(1).padStart(8)}%   $${r.finalVal.toFixed(2).padStart(8)}   trades=${String(r.trades).padStart(4)}   WR=${r.wr.toFixed(1).padStart(5)}%   maxDD=${r.maxDD.toFixed(1)}%   TP=${r.tpHits} SL=${r.slHits}\n`);
  }
})();
