// Stepped trailing stop for SOLBTC — same idea that improved SOLUSDT: wider trail early,
// tighten once deep in profit. BTC-denominated, continuous ~5yr, arm@6% (established best).
...
function trailForPeak(bestPct: number, stages: Stage[], base: number): number {
  let trail = base;
  for (const s of stages) if (bestPct >= s.threshold) trail = s.trailPp;
  return trail;
}

function runSim(c15: C[], c12h: C[], btcUsd: C[], baseTrail: number, stages: Stage[], label: string) {
  ...
  for (let i = 1; i < c15.length; i++) {
    ...
    if (mode === "SOL") {
      ...
      if (bestPct >= TRAIL_ARM_PCT) {
        const trail = trailForPeak(bestPct, stages, baseTrail);
...
(async () => {
  ...
  console.log(`\nSOLBTC · continuous ~5yr · stepped trailing stop test\n`);

  runSim(c15, c12h, btcUsd, 7.5, [], `[FLAT] arm6/trail7.5 (current "new")`);
  runSim(c15, c12h, btcUsd, 10, [{ threshold: 30, trailPp: 7.5 }], `10pp until 30%, then 7.5pp`);
  runSim(c15, c12h, btcUsd, 10, [{ threshold: 20, trailPp: 7.5 }], `10pp until 20%, then 7.5pp`);
  runSim(c15, c12h, btcUsd, 12, [{ threshold: 30, trailPp: 7.5 }], `12pp until 30%, then 7.5pp`);
  runSim(c15, c12h, btcUsd, 10, [{ threshold: 30, trailPp: 6 }], `10pp until 30%, then 6pp`);
  runSim(c15, c12h, btcUsd, 10, [{ threshold: 50, trailPp: 6 }], `10pp until 50%, then 6pp`);
})();
