// BCHFDUSD full available history (~2.75yr since Oct 2023), same strategy, with yearly
// breakdown, to see if the edge holds up on the actual pair over its whole life.
...
(async () => {
  // BCHFDUSD history starts ~2023-10-19; use that as our window start, with buffer before it
  const now = Date.now();
  const windowStart = new Date("2023-10-19T00:00:00Z").getTime();
  ...
  process.stdout.write(`Fetching ${symbol} 1m (full history)... `);
  ...
  console.log(`\nBCHFDUSD z-score, 1-min HL execution, NO timeout, TP=0.8%/SL=0.3% · full history (~2.75yr) · 0% fee\n`);
