// Tests a "stall exit" for SOLBTC: if entry gap was high (>= GAP_THRESH) and the trade
// hasn't reached MIN_GAIN within STALL_DAYS, exit early instead of waiting for the full
// trend-reversal signal. Targets specifically the "false confidence" pattern found in the
// loser audit: high-gap trades that fail to gain traction become the biggest losses.
// Layered on top of arm@6%/trail@7.5pp. BTC-denominated, continuous ~5yr.
...
function runSim(c15: C[], c12h: C[], btcUsd: C[], gapThresh: number | null, stallDays: number, minGain: number, label: string) {
  ...
  let entryPrice = 0, entryT = 0, entryGap = 0, bestPct = 0;
  let trades = 0, wins = 0, stallHits = 0;
  ...
  for (let i = 1; i < c15.length; i++) {
    ...
    if (mode === "BTC" && armedForSol && bullish && sloping) {
      entryPrice = price; entryT = t; entryGap = gapPct; bestPct = 0;
      solQty = btc / price; btc = 0; mode = "SOL"; armedForSol = false;
      const eq = solQty * price * usdPx;
      if (eq > peakUsd) peakUsd = eq;
    }

    let closeNow = false, isStall = false;
    if (mode === "SOL") {
      const curPct = (price - entryPrice) / entryPrice * 100;
      if (curPct > bestPct) bestPct = curPct;

      const daysSinceEntry = (t - entryT) / 86_400_000;
      if (gapThresh !== null && entryGap >= gapThresh && daysSinceEntry >= stallDays && bestPct < minGain) {
        closeNow = true; isStall = true;
  ...
  console.log(`${label.padEnd(40)}${(btcAccumRet>=0?"+":"")+btcAccumRet.toFixed(1).padStart(8)}%   $${finalUsd.toFixed(2).padStart(8)}   trades=${trades}   WR=${wr}%   maxDD=${maxDD.toFixed(1)}%   stallHits=${stallHits}`);
}
...
(async () => {
  ...
  console.log(`\nSOLBTC · continuous ~5yr · stall exit test (arm6/trail7.5)\n`);

  runSim(c15, c12h, btcUsd, null, 0, 0, `[BASELINE] no stall exit`);
  runSim(c15, c12h, btcUsd, 3, 5, 2, `gap>=3%, stall 5d, minGain 2%`);
  runSim(c15, c12h, btcUsd, 3, 7, 2, `gap>=3%, stall 7d, minGain 2%`);
  runSim(c15, c12h, btcUsd, 5, 5, 2, `gap>=5%, stall 5d, minGain 2%`);
  runSim(c15, c12h, btcUsd, 5, 7, 2, `gap>=5%, stall 7d, minGain 2%`);
  runSim(c15, c12h, btcUsd, 5, 10, 3, `gap>=5%, stall 10d, minGain 3%`);
})();
