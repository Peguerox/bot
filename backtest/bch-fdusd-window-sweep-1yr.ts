// Same window sweep (50 vs 5) on BCHFDUSD over 1 year, to decide whether to change the live
// paper bot's ZSCORE_WINDOW from 50 to 5.
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number, zscoreWindow: number) {
  ...
  while (i < candles5.length) {
    ...
    const month = new Date(candles5[i].t).toISOString().slice(0, 7);
    if (!(month in yearlyStart)) yearlyStart[month] = usd;
    yearlyEnd[month] = usd;
...
(async () => {
  ...
  console.log(`\nBCHFDUSD z-score window 50 vs 5 · 1yr · 1-min HL execution · no timeout · TP=0.8%/SL=0.3%\n`);
  for (const zw of [50, 5]) {
    const r = runSim(c5, c1, windowStart, zw);
    console.log(`window=${String(zw).padEnd(3)}${(r.ret>=0?"+":"")+r.ret.toFixed(1).padStart(9)}%   $${r.finalVal.toFixed(2).padStart(9)}   trades=${String(r.trades).padStart(5)}   WR=${r.wr.toFixed(1).padStart(5)}%   maxDD=${r.maxDD.toFixed(1)}%   TP=${r.tpHits} SL=${r.slHits}`);
  }
})();
