...
(async () => {
  ...
  process.stdout.write(`Fetching ${symbol} 1m (3mo, will take a while)... `);
  ...
  console.log(`\nSOLFDUSD hyper DCA ladder · 3mo · $${TOTAL_CAPITAL} cap, $${TRANCHE_USD}/tranche, TP=${TP_PCT}%, no SL, 1-min, no filter\n`);
  ...
  console.log(`Avg bars (1m candles) per completed ladder: ${r.avgLadderBars.toFixed(1)} (~${(r.avgLadderBars/60).toFixed(1)}h)`);
  if (r.stillOpen) {
