// SOLFDUSD, window=5, SL=0.1% fixed, TP swept, 1 year.
...
(async () => {
  ...
  console.log(`\n[TP=${TP_PCT}%/SL=0.1%] SOLFDUSD window=5 · 1yr\n`);
  console.log(`[TP=${TP_PCT}%] ${(r.ret>=0?"+":"")+r.ret.toFixed(1)}%   $${r.finalVal.toFixed(2)}   trades=${r.trades}   WR=${r.wr.toFixed(1)}%   maxDD=${r.maxDD.toFixed(1)}%   TP=${r.tpHits} SL=${r.slHits}${r.stillOpen?` (${r.stillOpen} still open)`:""}`);
  console.log(`By year:`);
