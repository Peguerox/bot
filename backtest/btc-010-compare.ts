...
function sim(
  ...
  tp: number, sl: number, thresh: number
) {
...
function row(label: string, r: ReturnType<typeof sim>, marker = "") {
  ...
  console.log(
    ...
    `  ${sign}${r.pct.toFixed(1)}%`.padStart(7) +
...
(async () => {
  ...
  console.log(`\n  Comparing symmetric settings  |  30d  |  $${ALLOC}/trade  |  market entry\n`);
  console.log("  Settings                  Trades  /day  TP hits   WR%    PF     PnL$    Ret%   MaxDD%");
  console.log("  " + "─".repeat(88));

  row("TP0.15% SL0.15% T0.15%", sim(aligned, usClose, glClose, 0.0015, 0.0015, 0.0015), "  ← current");
  row("TP0.10% SL0.10% T0.10%", sim(aligned, usClose, glClose, 0.001,  0.001,  0.001),  "  ← proposed");

  console.log();
  console.log("  Also testing mixed combinations:");
  console.log("  " + "─".repeat(88));
  row("TP0.10% SL0.15% T0.10%", sim(aligned, usClose, glClose, 0.001,  0.0015, 0.001));
  row("TP0.15% SL0.10% T0.10%", sim(aligned, usClose, glClose, 0.0015, 0.001,  0.001));
  row("TP0.10% SL0.10% T0.15%", sim(aligned, usClose, glClose, 0.001,  0.001,  0.0015));
  console.log();
})();
