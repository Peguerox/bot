// Pure Lag backtest across multiple coins on Binance.US
...
function runSim(
  ...
  return { pnl, pct: pnl / ALLOCATION * 100, wr, pf, trades, wins, missed, fillRate, maxDD, perDay: trades / days };
}

const coins = [
  { us: "XRPUSDT",  gl: "XRPUSDT"  },
  { us: "SOLUSDT",  gl: "SOLUSDT"  },
  { us: "ETHUSDT",  gl: "ETHUSDT"  },
  { us: "BTCUSDT",  gl: "BTCUSDT"  },
  { us: "XLMUSDT",  gl: "XLMUSDT"  },
];

(async () => {
  console.log(`\nPure Lag backtest · 30 days · Signal ≥${GL_THRESH*100}% + direction · TP=${TP_PCT*100}% SL=${SL_PCT*100}% MAX_HOLD=${MAX_HOLD} · $${ALLOCATION} compounding\n`);
  console.log("  Coin      Trades   /day   Fill%   WR%    PF      PnL$       Ret%    MaxDD%");
  console.log("  " + "─".repeat(75));

  for (const { us, gl } of coins) {
    const label = us.replace("USDT", "");
    process.stdout.write(`  Fetching ${label}...`);
    const [usCandles, glCandles] = await Promise.all([
      fetchKlines(BASE_US, us),
      fetchKlines(BASE_GL, gl),
    ]);
    const glMap   = new Map(glCandles.map(c => [c.time, c.close]));
    const aligned = usCandles.filter(c => glMap.has(c.time));
