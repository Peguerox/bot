// N=1 "buy after any green candle" momentum, 1 year, BTCFDUSD, comparing TP=0.3/SL=0.3 vs
// TP=0.8/SL=0.3. TP passed as CLI arg.
import dotenv from "dotenv";
dotenv.config({ path: ".env.local" });

const BASE = "https://data-api.binance.vision/api/v3";
const ALLOCATION_USD = 50;
const TP_PCT = parseFloat(process.argv[2] ?? "0.8");
const SL_PCT = 0.3;
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number) {
  ...
  while (i < candles5.length) {
    ...
    let allUp = true;
    for (let k = 0; k < N; k++) { if (prices[i - k] <= prices[i - k - 1]) { allUp = false; break; } }
...
(async () => {
  ...
  console.log(`\n[TP=${TP_PCT}%/SL=${SL_PCT}%] BTCFDUSD N=1 momentum · 1yr\n`);
