...
// tie-break. Takes exchange base URL, symbol, and years as CLI args so the exact same
// methodology can be run against Binance.US and Binance Global, any symbol, for direct
// apples-to-apples comparison (frequency, return, WR) across exchanges/pairs.
import dotenv from "dotenv";
dotenv.config({ path: ".env.local" });

const BASE   = process.argv[2] ?? "https://data-api.binance.vision/api/v3";
const SYMBOL = process.argv[3] ?? "SOLFDUSD";
const YEARS  = parseFloat(process.argv[4] ?? "1");
const TP_PCT = 1.0;
const SL_PCT = 0.1;
...
async function fetchKlines(symbol: string, interval: string, startMs: number, endMs: number): Promise<any[]> {
  ...
  while (from < endMs) {
    ...
    if (!res.ok) { console.log(`  fetch error ${res.status}`); return out; }
...
(async () => {
  ...
  const label = BASE.includes("binance.us") ? "Binance.US" : "Binance Global";
  process.stdout.write(`[${SYMBOL} @ ${label}] Fetching 1m (${YEARS}yr)... `);
  const raw1 = await fetchKlines(SYMBOL, "1m", fetchStart, now);
  const c1: C1[] = raw1.map(c => ({ t: +c[0], h: +c[2], l: +c[3], c: +c[4] }));
  console.log(`${c1.length}`);

  if (c1.length === 0) { console.log("NO DATA — check symbol exists on this exchange."); return; }

  const r = runSim(c1, windowStart);
  const tradesPerDay = r.trades / (YEARS * 365);
  console.log(`\n${SYMBOL} @ ${label} · 1-MIN entries · ${YEARS}yr · TP=${TP_PCT}%/SL=${SL_PCT}%\n`);
  console.log(`${(r.ret>=0?"+":"")+r.ret.toFixed(1)}%   $${r.finalVal.toFixed(2)}   trades=${r.trades} (${tradesPerDay.toFixed(1)}/day)   WR=${r.wr.toFixed(1)}%   maxDD=${r.maxDD.toFixed(1)}%   TP=${r.tpHits} SL=${r.slHits}${r.stillOpen?` (${r.stillOpen} still open)`:""}`);
})();
