...
const FETCH_BUFFER = 60 * 86_400_000; // extra history fetched so lookback works from day 1 of the sim
...
(async () => {
  ...
  console.log(`\nBasket rotation {BTC,SOL,ETH} — clean, consistent start date\n`);
  runSim(btcUsd, solUsd, ethUsd, simStart, now, 14, 7, `14d / 7d`);
  runSim(btcUsd, solUsd, ethUsd, simStart, now, 15, 7, `15d / 7d`);
  runSim(btcUsd, solUsd, ethUsd, simStart, now, 14, 8, `14d / 8d`);
  runSim(btcUsd, solUsd, ethUsd, simStart, now, 16, 7, `16d / 7d`);
})();
