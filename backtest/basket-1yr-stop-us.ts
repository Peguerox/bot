...
const FETCH_BUFFER = 30 * 86_400_000;
...
(async () => {
  ...
  console.log(`Sim window: ${new Date(simStart).toISOString().slice(0,10)} -> ${new Date(now).toISOString().slice(0,10)} (1yr)\n`);
  ...
  console.log(`\nBasket rotation + continuous hard stop — 1yr — refine around 20d/10d/-10%\n`);
  ...
  runSim(btcUsd, solUsd, ethUsd, simStart, now, 20, 10, -10, `20d/10d, -10% stop [found]`);
  runSim(btcUsd, solUsd, ethUsd, simStart, now, 20, 10, -11, `20d/10d, -11% stop`);
  runSim(btcUsd, solUsd, ethUsd, simStart, now, 20, 10, -12, `20d/10d, -12% stop`);
  runSim(btcUsd, solUsd, ethUsd, simStart, now, 18, 9, -10, `18d/9d, -10% stop`);
  runSim(btcUsd, solUsd, ethUsd, simStart, now, 22, 11, -10, `22d/11d, -10% stop`);
  runSim(btcUsd, solUsd, ethUsd, simStart, now, 20, 8, -10, `20d/8d, -10% stop`);
  runSim(btcUsd, solUsd, ethUsd, simStart, now, 20, 12, -10, `20d/12d, -10% stop`);
})();
