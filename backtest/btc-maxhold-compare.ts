// Compare MAX_HOLD values for BTC Pure Lag
...
const GL_THRESH = 0.0005, TP_PCT = 0.008, SL_PCT = 0.0015, ALLOCATION = 25, ENTRY_SLIP = 0.0002, CANCEL_DROP = 0.0015;
...
function runSim(aligned: { high: number; low: number; close: number }[], usClose: number[], glClose: number[], maxHold: number) {
  type Pos = { entry: number; tp: number; sl: number; hold: number; chasing: boolean; chasePrice: number };
  let bal = ALLOCATION, peak = ALLOCATION, maxDD = 0, trades = 0, wins = 0, missed = 0, cancelDrops = 0, gW = 0, gL = 0;
  ...
  for (let i = 1; i < aligned.length; i++) {
    ...
    if (pending !== null) {
      ...
      if (low <= pending) { pos = { entry: pending, tp: pending*(1+TP_PCT), sl: pending*(1-SL_PCT), hold: 0, chasing: false, chasePrice: 0 }; }
      else missed++;
    ...
    if (pos) {
      ...
      if (pos.chasing) {
        ...
        else if (pos.hold >= maxHold) { pos.chasing = true; pos.chasePrice = close; }
      }
      continue;
    }
  ...
  return { pnl, pct: pnl/ALLOCATION*100, wr, pf, trades, wins, cancelDrops, missed, maxDD, perDay: trades/days };
}
...
(async () => {
  ...
  console.log(`TP=${TP_PCT*100}%  SL=${SL_PCT*100}%  Thresh=${GL_THRESH*100}%  $${ALLOCATION} compounding\n`);
  console.log("  MaxHold   Trades  /day   WR%    PF     PnL$      Ret%   MaxDD%");
  console.log("  " + "─".repeat(62));

  for (const h of [1, 2, 3, 4, 5, 6, 8, 10, 15]) {
    const r = runSim(aligned, usClose, glClose, h);
