// 3-column comparison v2: Hold SOL vs Stop-loss only (-6%, current reference) vs
// Stop-loss + Trailing (arm@8%, trail@10pp) — same continuous ~5yr methodology.
...
function runSim(c15: C[], c12h: C[], useTrailing: boolean) {
  ...
  for (let i = 1; i < c15.length; i++) {
    ...
    if (mode === "SOL") {
      ...
      else if (useTrailing && bestPct >= TRAIL_ARM_PCT && (bestPct - curPct) >= TRAIL_PP) closeNow = true;
...
(async () => {
  ...
  const stopSnaps = runSim(c15, c12h, false);
  const trailSnaps = runSim(c15, c12h, true);

  console.log(`\n${"year".padEnd(8)}${"Hold SOL".padStart(12)}${"Stop-loss only (-6%)".padStart(24)}${`+ Trailing (arm8/trail10)`.padStart(28)}`);
  console.log("─".repeat(72));
  let holdMult = 1, stopMult = 1, trailMult = 1;
  for (let i = 0; i < stopSnaps.length; i++) {
    const h = holdSnaps[i], s = stopSnaps[i], tr = trailSnaps[i];
    const hRet = (h.endPx - h.startPx) / h.startPx * 100;
    const sRet = (s.endEq - s.startEq) / s.startEq * 100;
    const trRet = (tr.endEq - tr.startEq) / tr.startEq * 100;
    holdMult *= (1 + hRet/100); stopMult *= (1 + sRet/100); trailMult *= (1 + trRet/100);
    console.log(`${s.label.padEnd(8)}${(hRet>=0?"+":"")+hRet.toFixed(1).padStart(10)}%${(sRet>=0?"+":"")+sRet.toFixed(1).padStart(22)}%${(trRet>=0?"+":"")+trRet.toFixed(1).padStart(26)}%`);
  }
  console.log("─".repeat(72));
  const hTot = (holdMult-1)*100, sTot = (stopMult-1)*100, trTot = (trailMult-1)*100;
  console.log(`${"TOTAL".padEnd(8)}${(hTot>=0?"+":"")+hTot.toFixed(1).padStart(10)}%${(sTot>=0?"+":"")+sTot.toFixed(1).padStart(22)}%${(trTot>=0?"+":"")+trTot.toFixed(1).padStart(26)}%`);
  console.log(`\n(dollar terms on $50: Hold=$${(50*holdMult).toFixed(2)}  Stop-loss=$${(50*stopMult).toFixed(2)}  +Trailing=$${(50*trailMult).toFixed(2)})`);
})();
