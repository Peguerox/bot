// Tests EXIT-side changes (never touched before now — every prior test only changed entry).
// All variants keep the SAME entry: RSI arm 30 + EMA bullish + EMA7 sloping (the proven best).
//   [BASELINE] current exit: RSI arm 70 (cross down through it) + EMA bearish
//   [E1] SOLUSDT-style exit: EMA bearish + RSI < 50, no arming required — fires as soon as
//        both are true, doesn't wait for RSI to first pop above 70
//   [E2] baseline exit + a -5% hard stop-loss (informed by the MAE audit: no winner in 5yr
//        ever drew down more than -4.1%, so a -5% stop risks zero known winners)
//   [E3] E1 + the -5% stop combined
...
type ExitOpts = { altExit?: boolean; stopLoss?: boolean };

function runSim(c15: C[], c12h: C[], btcUsd: C[], label: string, opts: ExitOpts) {
  ...
  for (let i = 1; i < c15.length; i++) {
    ...
    if (!opts.altExit && rsi[i-1] > RSI_HIGH && rsi[i] <= RSI_HIGH && mode === "SOL" && !armedForBtc) armedForBtc = true;
    ...
    if (mode === "SOL") {
      ...
      if (opts.stopLoss && curPct <= STOP_PCT) { closeNow = true; isStop = true; }
      else if (opts.altExit) {
        if (bullish === false && rsi[i] < RSI_EXIT_ALT) closeNow = true;
      } else {
        if (armedForBtc && !bullish) closeNow = true;
      }
    }
  ...
  console.log(`  USD mark-to-market: $${finalUsd.toFixed(2)}  |  Trades: ${trades}  |  WR: ${wr}%  |  Max DD: ${maxDD.toFixed(2)}%${opts.stopLoss ? `  |  Stop-outs: ${stopHits}` : ""}`);
}
...
(async () => {
  ...
  console.log(`\n${"═".repeat(80)}`);
  console.log(` SOLBTC · continuous 5yr · ${dLabel} · Binance.US · entry always = slope filter`);
  console.log(`${"═".repeat(80)}`);

  runSim(c15, c12h, btcUsd, `[BASELINE] current exit: RSI arm 70 + EMA bearish`, {});
  runSim(c15, c12h, btcUsd, `[E1] SOLUSDT-style exit: EMA bearish + RSI<50, no arm`, { altExit: true });
  runSim(c15, c12h, btcUsd, `[E2] baseline exit + -5% stop-loss`, { stopLoss: true });
  runSim(c15, c12h, btcUsd, `[E3] E1 exit + -5% stop-loss`, { altExit: true, stopLoss: true });
})();
