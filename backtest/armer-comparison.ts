// Same "armed scalp" strategy (bet each candle for TP=1.0%/SL=0.1% while the favorable zone
// holds, stop opening new bets once it ends) tested with 4 different armers: Bollinger Bands
// (price below lower BB 20,2), VWAP (price below rolling 20-period VWAP), EMA (price below
// EMA20), RSI (RSI14 < 30). SOLFDUSD, 1 month.
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number, favorable: boolean[]) {
...
(async () => {
  ...
  const armers = [
    ...
    { name: "EMA20", fav: closes.map((c, i) => !isNaN(ema[i]) && c < ema[i]) },
    { name: "RSI<30", fav: rsi.map(r => !isNaN(r) && r < RSI_OVERSOLD) },
  ];

  console.log(`\nArmer comparison · SOLFDUSD · 1mo · TP=1.0%/SL=0.1%\n`);
  for (const a of armers) {
    const r = runSim(c5, c1, windowStart, a.fav);
    console.log(`${a.name.padEnd(16)}${(r.ret>=0?"+":"")+r.ret.toFixed(1).padStart(8)}%   trades=${String(r.trades).padStart(4)}   WR=${r.wr.toFixed(1).padStart(5)}%   maxDD=${r.maxDD.toFixed(1)}%`);
  }
})();
