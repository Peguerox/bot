...
(async () => {
  ...
  const pnl = bal - ALLOC;
  const pct = (pnl / ALLOC * 100).toFixed(2);
  const wr  = trades > 0 ? (wins / trades * 100).toFixed(1) : "0";
  const pf  = grossLoss > 0 ? (grossWin / grossLoss).toFixed(2) : "∞";

  console.log(`Pure-Lag BNB · BTC≥0.2% · BNB<0.1% · 1m · 1 month · Binance.US`);
  console.log(`TP=0.8%  SL=0.15%  MAX_HOLD=${MAX_HOLD}  $${ALLOC} compounding`);
  console.log(`─────────────────────────────────────────`);
  console.log(`PnL:           ${pnl >= 0 ? "+" : ""}$${pnl.toFixed(2)} (${pnl >= 0 ? "+" : ""}${pct}%)`);
  console.log(`Trades:        ${trades}  (${(trades / 30).toFixed(1)}/day)`);
