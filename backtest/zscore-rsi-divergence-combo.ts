// Combines z-score entry (window=5, z<=-2.0) AND RSI bullish divergence (10-candle window
// comparison) — both must be true. Same execution: TP=0.8%/SL=0.3%, 1-min HL, no timeout.
// SOLFDUSD, 3 months.
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number) {
  ...
  startIdx = Math.max(startIdx, RSI_PERIOD + 2 * WIN + 1, ZSCORE_WINDOW);
  ...
  while (i < candles5.length) {
    ...
    const divergence = currentIsNewLow && priceLowerLow && rsiHigherLow;
    const zSignal = zscores[i] <= Z_ENTRY;
    const signal = divergence && zSignal;
...
(async () => {
  ...
  console.log(`\nSOLFDUSD z-score(w5,-2.0) + RSI bullish divergence combo · 3mo · TP=0.8%/SL=0.3%\n`);
