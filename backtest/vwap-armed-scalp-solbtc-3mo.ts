...
// TP/SL (CLI args). 1-min HL execution. SOLBTC, 1 month.
import dotenv from "dotenv";
dotenv.config({ path: ".env.local" });

const BASE = "https://data-api.binance.vision/api/v3";
const ALLOCATION_BTC = 0.001;
const PERIOD = 20;
  ...
  while (i < candles5.length) {
    ...
    if (favorable) {
      ...
      if (exitPrice === null) {
        ...
        btc = qty * lastPrice; qty = 0; stillOpen++; break;
      }
...
(async () => {
  ...
  console.log(`\nSOLBTC VWAP-armed scalp · 1mo · TP=${TP_PCT}%/SL=${SL_PCT}%\n`);
  console.log(`${(r.ret>=0?"+":"")+r.ret.toFixed(1)}%   ${r.finalVal.toFixed(8)} BTC   trades=${r.trades}   WR=${r.wr.toFixed(1)}%   maxDD=${r.maxDD.toFixed(1)}%   TP=${r.tpHits} SL=${r.slHits}${r.stillOpen?` (${r.stillOpen} still open)`:""}`);
})();
