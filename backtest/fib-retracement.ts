// Fibonacci retracement strategy: over a rolling lookback window, find the swing low and
// swing high. If the low occurred BEFORE the high (confirming an up-swing structure), and
// price has now pulled back to/below the 61.8% retracement level (but hasn't broken the
// swing low), buy — betting on a bounce off the golden-ratio support level.
// Same execution: TP=0.8%/SL=0.3%, 1-min HL, no timeout. SOLFDUSD, 3 months.
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number) {
  ...
  startIdx = Math.max(startIdx, SWING_LOOKBACK + 1);
  ...
  while (i < candles5.length) {
    ...
    const window = closes.slice(i - SWING_LOOKBACK, i);
    let lowIdx = 0, highIdx = 0;
    for (let k = 1; k < window.length; k++) {
      if (window[k] < window[lowIdx]) lowIdx = k;
      if (window[k] > window[highIdx]) highIdx = k;
    }
    const swingLow = window[lowIdx];
    const swingHigh = window[highIdx];
    const upSwingStructure = lowIdx < highIdx; // low occurred before high -> up-move happened
    const fibLevel = swingHigh - (swingHigh - swingLow) * FIB_LEVEL;

    const signal = upSwingStructure && closePrice <= fibLevel && closePrice > swingLow;
...
(async () => {
  ...
  const candleFetchStart = windowStart - (SWING_LOOKBACK + 5) * CANDLE_MS;
  ...
  console.log(`\nSOLFDUSD Fibonacci 61.8% retracement bounce · 3mo · TP=0.8%/SL=0.3%\n`);
