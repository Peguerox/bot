// Window=5 z-score strategy across the full original 15-coin list, 1 month, USDT pairs
// (Binance Global), same execution method (1-min HL, no timeout, TP=0.8%/SL=0.3%).
...
(async () => {
  ...
  for (const coin of coins) {
    ...
    if (!raw5.length) { console.log(`no data`); continue; }
  ...
  console.log(`\nZ-score window=5 · 1mo · Binance Global USDT · 1-min HL execution · no timeout · TP=0.8%/SL=0.3%\n`);
  for (const r of results) {
    console.log(`${r.coin.padEnd(6)}${(r.ret>=0?"+":"")+r.ret.toFixed(1).padStart(8)}%   trades=${String(r.trades).padStart(4)}   WR=${r.wr.toFixed(1).padStart(5)}%   maxDD=${r.maxDD.toFixed(1)}%`);
  }
})();
