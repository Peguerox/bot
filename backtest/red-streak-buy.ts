// "Same side" momentum strategy: after N consecutive up 5m candles in a row (close > previous
// close), buy on the close of the Nth candle, betting on continuation (not mean reversion).
// Same execution/exit as the z-score bot for apples-to-apples comparison: TP +0.8%/SL -0.3%,
// no timeout, 1-min HL execution. N passed as CLI arg. BTCFDUSD, 1 month.
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number) {
  ...
  while (i < candles5.length) {
    ...
    let allDown = true;
    for (let k = 0; k < N; k++) {
      if (prices[i - k] >= prices[i - k - 1]) { allDown = false; break; }
    }
...
(async () => {
  ...
  console.log(`\n[N=${N}] BTCFDUSD "buy the dip" (red streak) (buy after ${N} red candles in a row) · 1mo · TP=0.8%/SL=0.3%\n`);
  console.log(`[N=${N}] ${(r.ret>=0?"+":"")+r.ret.toFixed(1)}%   $${r.finalVal.toFixed(2)}   trades=${r.trades}   WR=${r.wr.toFixed(1)}%   maxDD=${r.maxDD.toFixed(1)}%   TP=${r.tpHits} SL=${r.slHits}${r.stillOpen?` (${r.stillOpen} still open)`:""}`);
})();
