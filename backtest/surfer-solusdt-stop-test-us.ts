// Tests a stop-loss on SOLUSDT (informed by MAE audit: no winner in 5yr ever drew down more
// than -4.9%, and every loser that breached -5% went on to lose MORE than -5% naturally).
// Simulated properly (not just averaged) since exit-timing changes cascade to later trades.
...
function runSim(c15: C[], c12h: C[], stopPct: number | null, label: string) {
  ...
  console.log(`${label.padEnd(30)}${(totalRet>=0?"+":"")+totalRet.toFixed(1).padStart(10)}%   $${finalVal.toFixed(2).padStart(10)}   trades=${String(trades).padStart(3)}   WR=${wr.padStart(5)}%   maxDD=${maxDD.toFixed(1)}%${stopPct !== null ? `   stopHits=${stopHits}` : ""}`);
}
...
(async () => {
  ...
  console.log(`\nSOLUSDT · continuous ~5yr · stop-loss test · Binance.US\n`);

  runSim(c15, c12h, null, `[BASELINE] no stop`);
  runSim(c15, c12h, -5, `-5% stop`);
  runSim(c15, c12h, -6, `-6% stop`);
  runSim(c15, c12h, -7, `-7% stop`);
  runSim(c15, c12h, -8, `-8% stop`);
  runSim(c15, c12h, -10, `-10% stop`);
  runSim(c15, c12h, -15, `-15% stop`);
})();
