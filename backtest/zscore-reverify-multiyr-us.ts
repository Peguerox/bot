// Re-verifies the original single-asset z-score screen (SOL/ETC/LINK/UNI/DOT/ATOM) using the
// exact same close-only methodology as the original tests, but across 1yr/3yr/5yr windows
// instead of just 1yr, to check whether the earlier "SOL/ETC look great" result was a rolling-
// window artifact. Global data (data-api.binance.vision), 0% fee, same params as always.
...
function runSim(candles: C[], sliceFromMs: number): { ret: number; trades: number; wr: number; maxDD: number } {
...
(async () => {
  ...
  const coins = ["SOL", "ETC", "LINK", "UNI", "DOT", "ATOM"];
