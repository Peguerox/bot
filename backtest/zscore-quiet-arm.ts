// Detects unusually quiet stretches (rolling 20-candle volatility in the bottom 25% of its
// own trailing 100-reading distribution). Once a quiet stretch is confirmed, "arm" and take
// only the FIRST z-score signal (z<=-2.0) that fires afterward — then require a fresh quiet
// stretch before arming again. TP=0.8%/SL=0.3%, 1-min HL, no timeout. SOLFDUSD, 3 months.
...
const VOL_PERCENTILE_LOOKBACK = 100;
const QUIET_PERCENTILE = 0.25; // bottom 25% = quiet
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number) {
  ...
  const zscores: number[] = new Array(prices.length).fill(NaN);
  for (let i = ZSCORE_WINDOW; i < prices.length; i++) {
  ...
  const returns: number[] = new Array(prices.length).fill(NaN);
  for (let i = 1; i < prices.length; i++) returns[i] = (prices[i] - prices[i-1]) / prices[i-1];
  const vol: number[] = new Array(prices.length).fill(NaN);
  for (let i = VOL_LOOKBACK; i < prices.length; i++) {
  ...
  const isQuiet: boolean[] = new Array(prices.length).fill(false);
  for (let i = VOL_LOOKBACK + VOL_PERCENTILE_LOOKBACK; i < prices.length; i++) {
    const trailing = vol.slice(i - VOL_PERCENTILE_LOOKBACK, i).filter(v => !isNaN(v));
    const cutoff = percentile(trailing, QUIET_PERCENTILE);
    isQuiet[i] = vol[i] <= cutoff;
  }
  ...
  let m1Idx = 0;
  let armed = false; // true once we've seen a quiet stretch, waiting for the first signal

  let startIdx = candles5.findIndex(c => c.t >= windowStartMs);
  startIdx = Math.max(startIdx, ZSCORE_WINDOW, VOL_LOOKBACK + VOL_PERCENTILE_LOOKBACK);
  ...
  while (i < candles5.length) {
    ...
    if (isQuiet[i]) armed = true;

    const signal = armed && zscores[i] <= Z_ENTRY;

    if (signal) {
      armed = false; // consume the arm — only the first signal after a quiet stretch counts
...
(async () => {
  ...
  const candleFetchStart = windowStart - (ZSCORE_WINDOW + VOL_LOOKBACK + VOL_PERCENTILE_LOOKBACK + 5) * CANDLE_MS;
  ...
  console.log(`\nSOLFDUSD z-score, "first signal after quiet stretch" · 3mo · TP=0.8%/SL=0.3%\n`);
