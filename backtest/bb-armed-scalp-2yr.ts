...
const TP_PCT = parseFloat(process.argv[2] ?? "0.6");
const SL_PCT = 0.1;
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number) {
  ...
  return { ret, trades: tradesCount, wr, maxDD, finalVal, tpHits, slHits, stillOpen, monthly };
}
...
(async () => {
  ...
  process.stdout.write(`[TP=${TP_PCT}%] Fetching ${symbol} 1m (2yr, will take a while)... `);
  ...
  console.log(`\n[TP=${TP_PCT}%/SL=${SL_PCT}%] SOLFDUSD BB-armed scalp · 1yr\n`);
