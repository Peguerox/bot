// BullishHarami pattern + Bollinger Bands confirmation: only take the signal when price is at
// or below the lower Bollinger Band (20, 2 std) — i.e. the pattern fires while price is
// already statistically stretched to the downside, combining pattern + mean-reversion
// confirmation. Same execution: TP=0.8%/SL=0.3%, 1-min HL, no timeout. SOLFDUSD, 3 months.
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number) {
  ...
  startIdx = Math.max(startIdx, BB_PERIOD, 1);
  ...
  while (i < candles5.length) {
    ...
    const harami = i >= 1 && isRed(candles5[i-1]) && isGreen(candles5[i]) &&
      candles5[i].o >= candles5[i-1].c && candles5[i].c <= candles5[i-1].o;
    const belowLowerBB = !isNaN(bbLower[i]) && closePrice <= bbLower[i];
    const signal = harami && belowLowerBB;
    if (signal) signals++;
...
(async () => {
  ...
  console.log(`\nSOLFDUSD BullishHarami + below-lower-BB(20,2) · 3mo · TP=0.8%/SL=0.3%\n`);
