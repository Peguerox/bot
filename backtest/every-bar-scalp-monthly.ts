...
const YEARS = parseFloat(process.argv[4] ?? "5");
...
(async () => {
  ...
  process.stdout.write(`[TP=${TP_PCT}%/SL=${SL_PCT}%] Fetching ${symbol} 1m (will take a while)... `);
  ...
  console.log(`\nSOLFDUSD every-bar (no filter) · ${YEARS}yr requested (full available history) · TP=${TP_PCT}%/SL=${SL_PCT}%\n`);
