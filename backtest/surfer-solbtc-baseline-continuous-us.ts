// Original baseline (live bot logic, no slope filter), run as ONE continuous simulation —
// closes the gap from the last audit, which only reran the SLOPE-FILTER version continuously.
// This is the exact mirror of surfer-solbtc-slope-continuous-us.ts with ONE line different:
// the buy condition drops the "sloping" requirement (matches trigger/live-bot-surfer-solbtc.ts).
...
function runSim(c15: C[], c12h: C[], btcUsd: C[], label: string) {
  ...
  for (let i = 1; i < c15.length; i++) {
    ...
    const { bullish } = getTrend(t, price);

    // NO slope requirement here — matches trigger/live-bot-surfer-solbtc.ts exactly
  ...
  console.log(` BTC accumulated: ${startBtc.toFixed(8)} → ${finalBtc.toFixed(8)}  (${btcAccumRet >= 0 ? "+" : ""}${btcAccumRet.toFixed(1)}% coin-denominated, CONTINUOUS, no window resets)`);
...
(async () => {
  ...
  runSim(c15, c12h, btcUsd, `SOLBTC · Surfer BASELINE (no slope filter) · ${dLabel} (Binance.US, CONTINUOUS)`);
})();
