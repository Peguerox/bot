...
const SL_PCT = 0.0015, MAX_HOLD = 6, ALLOC = 25, GL_THRESH = 0.001;
...
function sim(aligned: { high: number; low: number; close: number }[], usClose: number[], glClose: number[], tp: number) {
  ...
  for (let i = 2; i < aligned.length; i++) {
    ...
    // Last closed candle signal (matches live bot)
    ...
    if (spread >= GL_THRESH && glRet > 0 && usRet < glRet) {
      ...
      pos = { entry, tp: entry*(1+tp), sl: entry*(1-SL_PCT), hold: 0, chasing: false, chasePrice: 0 };
    }
  }
...
(async () => {
  ...
  console.log(`\nSL=${SL_PCT*100}%  GL_THRESH=${GL_THRESH*100}%  Market entry  $${ALLOC} fixed/trade\n`);
  console.log("  TP       Trades  /day  TP hits   WR%    PF      PnL\$    Ret%   MaxDD%");
  console.log("  " + "─".repeat(70));

  for (const tp of [0.001, 0.0015, 0.002, 0.003, 0.004, 0.006, 0.008]) {
    const r = sim(aligned, usClose, glClose, tp);
    const pf = r.pf === Infinity ? "  inf" : r.pf.toFixed(2);
    const marker = tp === 0.004 ? " ← now" : "";
    console.log(
      `  ${(tp*100).toFixed(2)}%`.padEnd(9) +
      `${r.trades}`.padStart(7) +
      `  ${r.perDay.toFixed(1)}`.padStart(5) +
      `  ${r.tpHits}`.padStart(8) +
      `  ${r.wr.toFixed(1)}%`.padStart(6) +
      `  ${pf}`.padStart(6) +
      `  +$${r.pnl.toFixed(2)}`.padStart(8) +
