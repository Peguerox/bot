// SOLFDUSD, window=5, TP=1.0% fixed, SL swept, 1 month.
...
const Z_ENTRY = -2.0;
const TP_PCT = 1.0;
const SL_PCT = parseFloat(process.argv[2] ?? "0.3");
...
(async () => {
  ...
  process.stdout.write(`[SL=${SL_PCT}%] Fetching ${symbol} 5m... `);
  const raw5 = await fetchKlines(symbol, "5m", candleFetchStart, now);
  const c5: C5[] = raw5.map(c => ({ t: +c[0], c: +c[4] }));
  console.log(`${c5.length}`);
  process.stdout.write(`[SL=${SL_PCT}%] Fetching ${symbol} 1m (1yr, will take a while)... `);
  ...
  console.log(`\n[TP=1.0%/SL=${SL_PCT}%] SOLFDUSD window=5 · 1mo\n`);
  console.log(`[SL=${SL_PCT}%] ${(r.ret>=0?"+":"")+r.ret.toFixed(1)}%   $${r.finalVal.toFixed(2)}   trades=${r.trades}   WR=${r.wr.toFixed(1)}%   maxDD=${r.maxDD.toFixed(1)}%   TP=${r.tpHits} SL=${r.slHits}${r.stillOpen?` (${r.stillOpen} still open)`:""}`);
})();
