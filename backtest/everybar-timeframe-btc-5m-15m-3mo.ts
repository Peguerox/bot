...
(async () => {
  const now = Date.now();
  const windowStart = now - 182 * 24 * 60 * 60 * 1000; // 6mo
  const candleFetchStart = windowStart - 20 * 60 * 1000;

  const symbol = "BTCUSDT";
  process.stdout.write(`Fetching ${symbol} 1m execution data (6mo)... `);
  ...
  console.log(`\nEvery-bar timeframe comparison · BTCUSDT @ Binance.US · 5yr · TP=1.0%/SL=0.1%\n`);
  for (const { name, r } of results) {
    console.log(`${name.padEnd(6)}${(r.ret>=0?"+":"")+r.ret.toFixed(1).padStart(9)}%   trades=${String(r.trades).padStart(6)}   WR=${r.wr.toFixed(1).padStart(5)}%   maxDD=${r.maxDD.toFixed(1)}%${r.stillOpen?` (${r.stillOpen} still open)`:""}`);
    console.log(`   Year   Return       Trades   WR      TP hits   SL hits`);
    for (const y of r.yearly) {
      console.log(`   ${y.year}   ${(y.ret>=0?"+":"")+y.ret.toFixed(1)}%`.padEnd(20) + `${String(y.trades).padStart(6)}   ${y.wr.toFixed(1).padStart(5)}%   ${String(y.tp).padStart(7)}   ${String(y.sl).padStart(7)}`);
    }
  }
})();
