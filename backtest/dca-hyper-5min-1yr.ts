...
(async () => {
  ...
  process.stdout.write(`Fetching ${symbol} 5m (1yr, will take a while)... `);
  const raw1 = await fetchKlines(symbol, "5m", fetchStart, now);
  const c1: C1[] = raw1.map(c => ({ t: +c[0], h: +c[2], l: +c[3], c: +c[4] }));
  console.log(`${c1.length}`);

  const r = runSim(c1, windowStart);
  console.log(`\nSOLFDUSD hyper DCA ladder · 1yr · $${TOTAL_CAPITAL} cap, $${TRANCHE_USD}/tranche, TP=${TP_PCT}%, no SL, 5-min, no filter\n`);
