...
const THRESHOLDS = [0.003, 0.002, 0.001]; // 0.3%, 0.2%, 0.1%
...
function run(btc: { time: number; high: number; low: number; close: number }[], coin: { time: number; high: number; low: number; close: number }[], btcThresh: number) {
  ...
  for (let i = 1; i < aligned.length; i++) {
    ...
    if (btcRet >= btcThresh && coinRet < COIN_MAX) {
...
(async () => {
  ...
  console.log(`Pure-Lag BNB · BNB<0.1% · TP=0.8% · SL=0.15% · 1m · 1 month · Binance.US\n`);
  console.log(`${"Threshold".padEnd(12)} ${"PnL".padStart(8)}  ${"Ret%".padStart(7)}  ${"Trades".padStart(6)}  ${"T/day".padStart(5)}  ${"WR%".padStart(6)}  ${"PF".padStart(5)}  ${"MaxDD".padStart(6)}  ${"EndBal".padStart(8)}`);
  console.log("─".repeat(80));

  for (const thresh of THRESHOLDS) {
    const r = run(btcRaw, bnbRaw, thresh);
    const sign = r.pnl >= 0 ? "+" : "";
    console.log(
      `BTC≥${(thresh*100).toFixed(1)}%`.padEnd(12) +
