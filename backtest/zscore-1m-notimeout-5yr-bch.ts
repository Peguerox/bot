// Same BCH sim (1-min HL execution, no timeout, TP=0.8%/SL=0.3%) extended to 5 years, with
// yearly breakdown.
...
(async () => {
  const now = Date.now();
  const windowStart = now - 5 * 365 * 24 * 60 * 60 * 1000; // 5yr
  ...
  process.stdout.write(`Fetching ${symbol} 1m (5yr, this will take a while)... `);
  ...
  console.log(`\nBCH z-score, 1-min HL execution, NO timeout, TP=0.8%/SL=0.3% · 5yr · Binance Global · 0% fee\n`);
