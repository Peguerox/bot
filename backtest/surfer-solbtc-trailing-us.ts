// Tests a trailing stop on SOLBTC (informed by the MFE audit — same giveback pattern found
// on SOLUSDT). Keeps the existing slope-filter entry and RSI-arm-70 exit as fallback; adds
// a trailing exit once a trade is up enough. Deliberately NOT combined with a hard stop-loss
// floor, since that was already tested and hurt SOLBTC (losers there tend to recover before
// the natural exit). This isolates the trailing mechanism on its own. BTC-denominated, same
// continuous ~5yr methodology as the rest of the SOLBTC investigation.
...
function runSim(c15: C[], c12h: C[], btcUsd: C[], armPct: number | null, trailPct: number | null, label: string) {
  ...
  let entryBtc = 0, entryPrice = 0, bestPct = 0;
  let trades = 0, wins = 0, trailHits = 0;
  ...
  for (let i = 1; i < c15.length; i++) {
    ...
    if (mode === "BTC" && armedForSol && bullish && sloping) {
      entryBtc = btc; entryPrice = price; bestPct = 0;
      solQty   = btc / price;
      btc      = 0;
      mode     = "SOL";
      armedForSol = false;
    ...
    let closeNow = false, isTrail = false;
    if (mode === "SOL") {
      const curPct = (price - entryPrice) / entryPrice * 100;
      if (curPct > bestPct) bestPct = curPct;

      if (trailPct !== null && armPct !== null && bestPct >= armPct && (bestPct - curPct) >= trailPct) {
        closeNow = true; isTrail = true;
      } else if (armedForBtc && !bullish) {
        closeNow = true;
      }
    }
  ...
  console.log(`${label.padEnd(32)}${(btcAccumRet>=0?"+":"")+btcAccumRet.toFixed(1).padStart(8)}%   $${finalUsd.toFixed(2).padStart(8)}   trades=${String(trades).padStart(3)}   WR=${wr.padStart(5)}%   maxDD=${maxDD.toFixed(1)}%   trailHits=${trailHits}`);
}
...
(async () => {
  ...
  console.log(`\nSOLBTC · continuous ~5yr · trailing stop test (BTC-denominated) · Binance.US\n`);

  runSim(c15, c12h, btcUsd, null, null, `[BASELINE] slope filter, no trailing`);
  runSim(c15, c12h, btcUsd, 6, 10,  `arm@+6%, trail 10pp [conservative]`);
  runSim(c15, c12h, btcUsd, 6, 7.5, `arm@+6%, trail 7.5pp [riskier, near cliff]`);
  runSim(c15, c12h, btcUsd, 5.5, 10,  `arm@+5.5%, trail 10pp`);
  runSim(c15, c12h, btcUsd, 5.5, 7.5, `arm@+5.5%, trail 7.5pp`);
  runSim(c15, c12h, btcUsd, 5.5, 8,   `arm@+5.5%, trail 8pp`);
})();
