// 3 consecutive red candles, THEN a confirming green candle (the 4th) -> buy at that green
// candle's close. Different from the earlier "red streak" test, which bought at the close of
// the 3rd red candle itself with no green confirmation. Same execution: TP=0.8%/SL=0.3%,
// 1-min HL, no timeout. SOLFDUSD, 1 month first.
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number) {
  ...
  while (i < candles5.length) {
    ...
    const signal = isRed(candles5[i-3]) && isRed(candles5[i-2]) && isRed(candles5[i-1]) && isGreen(candles5[i]);
    if (signal) signals++;
...
(async () => {
  ...
  console.log(`\nSOLFDUSD 3-red-then-green · 1mo · TP=0.8%/SL=0.3%\n`);
