// Same volume signal screen, 3 months, with monthly breakdown for each signal.
...
const signals: Signal[] = [
  ...
  { name: "VolumeDryUpBreakout", check: (c, i) => {
      const recentAvg = (avgVol(c, i-1, 5) + avgVol(c, i, 5)) / 2;
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number, check: (c5: C5[], i: number) => boolean) {
  ...
  return { ret, trades: tradesCount, wr, maxDD, finalVal, tpHits, slHits, stillOpen, signalCount, monthly };
}
...
(async () => {
  ...
  console.log(`\nVolume-based signal screen · SOLFDUSD · 3mo · TP=0.8%/SL=0.3%\n`);
  for (const s of signals) {
    const r = runSim(c5, c1, windowStart, s.check);
    console.log(`${s.name}: ${(r.ret>=0?"+":"")+r.ret.toFixed(1)}%   signals=${r.signalCount}   trades=${r.trades}   WR=${r.wr.toFixed(1)}%   maxDD=${r.maxDD.toFixed(1)}%`);
