// Deeper pattern audit — computes correlation (Pearson + Spearman rank) between entry
// features and trade outcome (pnl%), instead of just group averages (which the last pass
// showed get dominated by 2-3 outlier trades). Adds 2 new candidate features:
//   - depthFromHigh90d: how far below the trailing 90-day high the entry price is (a
//     "how beaten down is it" depth measure, distinct from the 60d directional trend)
//   - volatility12h: recent realized volatility on 12h candles at entry (regime proxy)
// Also reports median (not just mean) winner/loser splits, which is more robust to the
// fat right tail this strategy has. Read-only, does not touch live bots.
...
// realized volatility: stdev of 12h log returns over trailing 14 periods, as %
function volAt(c12h: {t:number;c:number}[], t: number): number {
  let idx = -1;
  for (let i = c12h.length - 1; i >= 0; i--) { if (c12h[i].t <= t) { idx = i; break; } }
  if (idx < 14) return NaN;
  const rets: number[] = [];
  for (let j = idx - 13; j <= idx; j++) rets.push(Math.log(c12h[j].c / c12h[j-1].c));
...
type Trade = {
  entryTime: string; pnlPct: number;
  entryRsi: number; emaGapPct: number; slopeStrength: number;
  macroTrend60d: number; daysSincePrevClose: number;
  depthFromHigh90d: number; volatility12h: number;
};
...
function runSim(c15: C[], c12h: C[]): Trade[] {
  ...
  let entryIdx = 0, entryRsi = 0, entryGapPct = 0, entrySlope = 0;
  let lastCloseTime = 0;
  const trades: Trade[] = [];
  ...
  for (let i = 1; i < c15.length; i++) {
    ...
    if (mode === "BTC" && armedForSol && bullish && sloping) {
      entryIdx = i; entryRsi = rsi[i]; entryGapPct = gapPct; entrySlope = slopeStrength;
...
(async () => {
  ...
  const trades = runSim(c15, c12h).filter(t => !isNaN(t.macroTrend60d) && !isNaN(t.volatility12h));
  ...
  const features: [string, number[]][] = [
    ...
    ["volatility12h",   trades.map(t => t.volatility12h)],
  ];
  ...
  const sorted = [...trades].sort((a,b) => b.pnlPct - a.pnlPct).slice(0, 15);
  console.log(`\nTop 15 trades — checking if a depthFromHigh90d filter would have blocked them:`);
  console.log(`${"entry".padEnd(12)}${"pnl%".padStart(8)}${"depthFromHigh90d".padStart(20)}`);
  for (const t of sorted) {
    console.log(`${t.entryTime.padEnd(12)}${(t.pnlPct>=0?"+":"")+t.pnlPct.toFixed(1).padStart(6)}%${t.depthFromHigh90d.toFixed(1).padStart(19)}%`);
  }
})();
