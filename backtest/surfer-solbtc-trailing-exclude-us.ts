// Logs every trade for the conservative (arm6/trail10) and riskier (arm6/trail7.5) SOLBTC
// trailing-stop configs, then recomputes each config's compounded total with its single
// biggest trade (the 2023 SOL/BTC supercycle one) removed — to see how much of the "risky"
// config's edge depends on that one trade versus the conservative config.
...
function runSim(c15: C[], c12h: C[], armPct: number, trailPct: number): Trade[] {
  ...
  for (let i = 1; i < c15.length; i++) {
    ...
    if (mode === "SOL") {
      ...
      if (bestPct >= armPct && (bestPct - curPct) >= trailPct) closeNow = true;
...
(async () => {
  ...
  const conservative = runSim(c15, c12h, 6, 10);
  const risky        = runSim(c15, c12h, 6, 7.5);

  for (const [label, trades] of [["Conservative (arm6/trail10)", conservative], ["Riskier (arm6/trail7.5)", risky]] as [string, Trade[]][]) {
