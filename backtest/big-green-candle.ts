// Buy after a single 5m candle that moved open->close by at least a threshold % (a "big" green
// candle), not just any up candle. Same execution/exit as everything else: TP +0.8%/SL -0.3%,
// no timeout, 1-min HL execution. BTCFDUSD, 1 month.
...
const MIN_MOVE_PCT = parseFloat(process.argv[2] ?? "0.5");
...
(async () => {
  ...
  console.log(`\n[min=${MIN_MOVE_PCT}%] BTCFDUSD "big green candle" (open->close >= ${MIN_MOVE_PCT}%) · 1mo · TP=0.8%/SL=0.3%\n`);
  console.log(`[min=${MIN_MOVE_PCT}%] ${(r.ret>=0?"+":"")+r.ret.toFixed(1)}%   $${r.finalVal.toFixed(2)}   trades=${r.trades}   WR=${r.wr.toFixed(1)}%   maxDD=${r.maxDD.toFixed(1)}%   TP=${r.tpHits} SL=${r.slHits}${r.stillOpen?` (${r.stillOpen} still open)`:""}`);
})();
