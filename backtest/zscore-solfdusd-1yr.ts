...
(async () => {
  const now = Date.now();
  const start = now - 365 * 24 * 60 * 60 * 1000; // 1yr

  const symbols = ["SOLFDUSD", "SOLUSDT"];
  const results: { coin: string; ret: number; trades: number; wr: number; maxDD: number }[] = [];
  for (const coin of symbols) {
    process.stdout.write(`Fetching ${coin} 5m... `);
    const candles = await fetchKlines(coin, "5m", start, now);
