// Sweeps RSI arm thresholds for SOLBTC (currently 30/70) — never tested before. Layered on
// top of arm@6%/trail@7.5pp. BTC-denominated, continuous ~5yr. Read-only, does not touch
// live bots.
...
function runSim(c15: C[], c12h: C[], btcUsd: C[], rsiLow: number, rsiHigh: number, label: string) {
  ...
  for (let i = 1; i < c15.length; i++) {
    ...
    if (rsi[i-1] < rsiLow && rsi[i] >= rsiLow && mode === "BTC" && !armedForSol) armedForSol = true;
    if (rsi[i-1] > rsiHigh && rsi[i] <= rsiHigh && mode === "SOL" && !armedForBtc) armedForBtc = true;
  ...
  console.log(`${label.padEnd(24)}${(btcAccumRet>=0?"+":"")+btcAccumRet.toFixed(1).padStart(8)}%   $${finalUsd.toFixed(2).padStart(8)}   trades=${trades}   WR=${wr}%   maxDD=${maxDD.toFixed(1)}%`);
}
...
(async () => {
  ...
  console.log(`\nSOLBTC · continuous ~5yr · RSI threshold sweep (arm6/trail7.5)\n`);

  runSim(c15, c12h, btcUsd, 30, 70, `[BASELINE] 30/70`);
  runSim(c15, c12h, btcUsd, 25, 70, `25/70`);
  runSim(c15, c12h, btcUsd, 35, 70, `35/70`);
  runSim(c15, c12h, btcUsd, 30, 65, `30/65`);
  runSim(c15, c12h, btcUsd, 30, 75, `30/75`);
  runSim(c15, c12h, btcUsd, 25, 75, `25/75`);
  runSim(c15, c12h, btcUsd, 35, 65, `35/65`);
  runSim(c15, c12h, btcUsd, 20, 80, `20/80`);
})();
