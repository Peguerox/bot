// Surfer on SOLUSDT — same logic as live SOLBTC bot but USD-denominated
// Entry: RSI(14) 15m crosses UP through 30 + EMA7 > EMA25 on 12h (liveMode)
// Exit:  RSI(14) 15m crosses DOWN through 70 + EMA7 < EMA25 on 12h (liveMode)
// Hold indefinitely between signals. No TP/SL.
...
function runSim(c15: C[], c12h: C[], label: string, trailingStopPct: number | null = null, showMonthly = false) {
  ...
  // build 12h EMA lookup with liveMode adjustment
  ...
  let armedBuy = false, armedSell = false;
  let candlesAbove30 = 0;
  ...
  // monthly snapshot: { ym, startEq, endEq }
  ...
  for (let i = 1; i < c15.length; i++) {
    ...
    // track consecutive candles above 30 while in USDT mode
    if (mode === "USDT") {
      if (rsi[i] >= RSI_LOW) candlesAbove30++;
      else candlesAbove30 = 0;
    }

    // arm buy: RSI crosses up through 30
    if (rsi[i-1] < RSI_LOW && rsi[i] >= RSI_LOW && mode === "USDT") armedBuy = true;

    // fire buy: armed + EMA bullish + EMA7 sloping up
    const { bullish, slowEma, fastSloping } = getTrend(t, price);
    ...
    // exit: EMA death cross + RSI below 50 (confirms downtrend, avoids whipsaws)
    ...
    // track DD while in position; fire trailing stop if enabled
    if (mode === "SOL") {
      const eq = solQty * price;
      if (eq > peak) peak = eq;
...
(async () => {
  const now    = Date.now();
  const starts = [
    { s: now - LOOKBACK,     e: now,              label: "Jun 2025 – Jun 2026" },
    { s: now - 2 * LOOKBACK, e: now - LOOKBACK,   label: "Jun 2024 – Jun 2025" },
    { s: now - 3 * LOOKBACK, e: now - 2*LOOKBACK, label: "Jun 2023 – Jun 2024" },
    { s: now - 4 * LOOKBACK, e: now - 3*LOOKBACK, label: "Jun 2022 – Jun 2023" },
