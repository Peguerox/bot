...
const GL_THRESH = 0.0005, SL_PCT = 0.0015, MAX_HOLD = 6, ALLOCATION = 25, ENTRY_SLIP = 0.0002, CANCEL_DROP = 0.0015;
...
function runSim(aligned: { high: number; low: number; close: number }[], usClose: number[], glClose: number[], tpPct: number) {
  type Pos = { entry: number; tp: number; sl: number; hold: number; chasing: boolean; chasePrice: number };
  let bal = ALLOCATION, peak = ALLOCATION, maxDD = 0, trades = 0, wins = 0, tpHits = 0, gW = 0, gL = 0, tpProfit = 0;
  ...
  for (let i = 1; i < aligned.length; i++) {
    ...
    if (pending !== null) {
      if (low <= pending * (1 - CANCEL_DROP)) { pending = null; continue; }
      if (low <= pending) pos = { entry: pending, tp: pending*(1+tpPct), sl: pending*(1-SL_PCT), hold: 0, chasing: false, chasePrice: 0 };
    ...
    if (pos) {
      ...
      if (pos.chasing) {
        ...
        else if (high >= pos.tp) { const pnl=(pos.tp-pos.entry)*qty; bal+=pnl; trades++; wins++; tpHits++; tpProfit+=pnl; gW+=pnl; if(bal>peak)peak=bal; if((peak-bal)/peak*100>maxDD)maxDD=(peak-bal)/peak*100; pos=null; }
    ...
    if (spread >= GL_THRESH && glRet > 0 && usRet < glRet) pending = close * (1 + ENTRY_SLIP);
  }

  const pnl=bal-ALLOCATION, wr=trades>0?wins/trades*100:0, pf=gL>0?gW/gL:Infinity, days=aligned.length/60/24;
  return { pnl, pct: pnl/ALLOCATION*100, wr, pf, trades, tpHits, tpProfit, maxDD, perDay: trades/days };
}
...
(async () => {
  ...
  console.log(`SL=${SL_PCT*100}%  MAX_HOLD=${MAX_HOLD}  $${ALLOCATION} compounding\n`);
  console.log("  TP       Trades  /day   TP hits  TP profit   WR%    PF      PnL$      Ret%   MaxDD%");
