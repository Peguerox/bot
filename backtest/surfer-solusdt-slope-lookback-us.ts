// Sweeps the slope filter's lookback on SOLUSDT (mirrors the SOLBTC sweep), layered on top
// of the -6% stop-loss which is now the reference baseline for this bot. Current live setting
// compares EMA7 to EMA7 one 12h candle back; this tests 2/3/4/6 candles back too.
...
function runSim(c15: C[], c12h: C[], lookbackCandles: number, label: string) {
  ...
  console.log(`${label.padEnd(38)}${(totalRet>=0?"+":"")+totalRet.toFixed(1).padStart(9)}%   $${finalVal.toFixed(2).padStart(9)}   trades=${String(trades).padStart(3)}   WR=${wr.padStart(5)}%   maxDD=${maxDD.toFixed(1)}%`);
}
...
(async () => {
  ...
  console.log(`\nSOLUSDT · continuous ~5yr · slope lookback sweep (with -6% stop baseline) · Binance.US\n`);

  for (const n of [1, 2, 3, 4, 6]) {
    const hrs = n * 12;
    runSim(c15, c12h, n, `lookback=${n} candle(s) (${hrs}h)${n===1 ? " [current live, +stop]" : ""}`);
  }
})();
