// Z-score strategy (window=5, TP=0.8%/SL=0.3%, same as the base config across this session)
// filtered by volatility regime: rolling 20-candle realized volatility compared against its
// own trailing 100-reading median (dynamic, no lookahead) to classify each candle as
// high-vol or low-vol. Tests baseline (no filter) vs high-vol-only vs low-vol-only.
...
function runSim(candles5: C5[], candles1: C1[], windowStartMs: number, regime: "all" | "high" | "low") {
  ...
  const returns: number[] = new Array(candles5.length).fill(NaN);
  for (let i = 1; i < candles5.length; i++) returns[i] = (prices[i] - prices[i-1]) / prices[i-1];

  const vol: number[] = new Array(candles5.length).fill(NaN);
  for (let i = VOL_LOOKBACK; i < candles5.length; i++) {
    const window = returns.slice(i - VOL_LOOKBACK + 1, i + 1);
  ...
  const volRegimeHigh: boolean[] = new Array(candles5.length).fill(false);
  for (let i = VOL_LOOKBACK + VOL_MEDIAN_LOOKBACK; i < candles5.length; i++) {
    const trailingVols = vol.slice(i - VOL_MEDIAN_LOOKBACK, i).filter(v => !isNaN(v));
    const m = median(trailingVols);
    volRegimeHigh[i] = vol[i] > m;
  }
  ...
  startIdx = Math.max(startIdx, ZSCORE_WINDOW, VOL_LOOKBACK + VOL_MEDIAN_LOOKBACK);
  ...
  while (i < candles5.length) {
    ...
    let regimeOk = true;
    if (regime === "high") regimeOk = volRegimeHigh[i];
    else if (regime === "low") regimeOk = !volRegimeHigh[i];

    if (zscores[i] <= Z_ENTRY && regimeOk) {
...
(async () => {
  ...
  const candleFetchStart = windowStart - (ZSCORE_WINDOW + VOL_LOOKBACK + VOL_MEDIAN_LOOKBACK + 5) * CANDLE_MS;

  const coin = process.argv[2] ?? "SOL";
  const symbol = `${coin}FDUSD`;
  ...
  console.log(`\n${symbol} z-score + volatility regime filter · 1mo · TP=0.8%/SL=0.3%\n`);
  for (const regime of ["all", "high", "low"] as const) {
    const r = runSim(c5, c1, windowStart, regime);
    console.log(`${regime.padEnd(6)}${(r.ret>=0?"+":"")+r.ret.toFixed(1).padStart(8)}%   trades=${String(r.trades).padStart(4)}   WR=${r.wr.toFixed(1).padStart(5)}%   maxDD=${r.maxDD.toFixed(1)}%`);
  }
})();
