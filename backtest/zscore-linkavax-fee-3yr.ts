...
function runSim(pair: { t: number; a: number; b: number }[], label: string, feePctPerSide: number = 0) {
  ...
  for (let i = ZSCORE_WINDOW; i < pair.length; i++) {
    ...
    if (!inTrade && zscores[i] <= Z_ENTRY) {
      ...
      qty = (usd * (1 - feePctPerSide / 100)) / price; usd = 0; inTrade = true;
    }
    if (inTrade) {
      ...
      if (closeNow) {
        ...
        trades++; if (usd > qty * entryPrice) wins++; qty = 0; inTrade = false;
      }
    }
...
(async () => {
  ...
  process.stdout.write(`Fetching LINKUSDT 5m (3yr)... `); const link = await fetchKlines("LINKUSDT", "5m", start, now); console.log(`${link.length}`);
  process.stdout.write(`Fetching AVAXUSDT 5m (3yr)... `); const avax = await fetchKlines("AVAXUSDT", "5m", start, now); console.log(`${avax.length}`);
  const aligned = alignByTimestamp(link, avax);

  console.log(`\nLINK/AVAX Z-score · 3yr · global data · fee comparison\n`);
  runSim(aligned, `0% fee (as before)`, 0);
  runSim(aligned, `0.1% fee/side`, 0.1);
})();
