...
function runSim(candles: C[], candles1: C1[], windowStartMs: number, candleMs: number, SL_PCT: number) {
  ...
  const yearlyWins: Record<string, number> = {};
  const yearlyTP: Record<string, number> = {};
  const yearlySL: Record<string, number> = {};
  ...
  while (i < candles.length) {
    ...
    if (!(year in yearlyStart)) yearlyStart[year] = usd + qty * closePrice;
    yearlyEnd[year] = usd + qty * closePrice;
    yearlyTrades[year] = yearlyTrades[year] ?? 0;
    yearlyWins[year] = yearlyWins[year] ?? 0;
    yearlyTP[year] = yearlyTP[year] ?? 0;
    yearlySL[year] = yearlySL[year] ?? 0;
    ...
    yearlyTrades[year]++;
    if (usd > qty * entryPrice) yearlyWins[year]++;
    if (reason === "TP") yearlyTP[year]++; else yearlySL[year]++;
    qty = 0;
  ...
  const yearly = Object.keys(yearlyStart).sort().map(y => ({
    ...
    trades: yearlyTrades[y] ?? 0,
    wr: yearlyTrades[y] ? (yearlyWins[y] / yearlyTrades[y] * 100) : 0,
    tp: yearlyTP[y] ?? 0,
    sl: yearlySL[y] ?? 0,
  }));
...
(async () => {
  ...
  process.stdout.write(`Fetching ${symbol} 1m execution data (1yr)... `);
  const raw1 = await fetchKlines(symbol, "1m", windowStart, now);
  const c1: C1[] = raw1.map(c => ({ t: +c[0], h: +c[2], l: +c[3], c: +c[4] }));
  console.log(`${c1.length}`);

  process.stdout.write(`Fetching ${symbol} 5m entry candles... `);
  const raw5 = await fetchKlines(symbol, "5m", candleFetchStart, now);
  const c5: C[] = raw5.map((x: any) => ({ t: +x[0], c: +x[4] }));
  console.log(`${c5.length}`);

  const results: { name: string; r: ReturnType<typeof runSim> }[] = [];
  for (const slPct of [0.1, 0.2]) {
    const r = runSim(c5, c1, windowStart, 5 * 60 * 1000, slPct);
    results.push({ name: `SL=${slPct}%`, r });
  }

  console.log(`\nEvery-bar SL comparison · BTCUSDT @ Binance.US · 5-min · 1yr · TP=${TP_PCT}%\n`);
  for (const { name, r } of results) {
    console.log(`${name.padEnd(8)}${(r.ret>=0?"+":"")+r.ret.toFixed(1).padStart(9)}%   trades=${String(r.trades).padStart(6)}   WR=${r.wr.toFixed(1).padStart(5)}%   maxDD=${r.maxDD.toFixed(1)}%   TP=${r.tpHits} SL=${r.slHits}${r.stillOpen?` (${r.stillOpen} still open)`:""}`);
  }
})();
