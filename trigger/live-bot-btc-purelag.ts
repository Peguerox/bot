...
import {
  getBtcPosition as getPosition, openBtcPendingEntry as openPendingEntry, setBtcEntryFilled as setEntryFilled,
  incrementBtcHold as incrementHold, setBtcChasing as setChasing, updateBtcChaseFloor as updateChaseFloor,
  closeBtcPosition as closePosition, logBtcRun as logRun, getBtcSettings as getSettings,
  setBtcBaseline as setBaseline, updateBtcBalance as updateBalance, updateBtcTotal as updateTotal,
  setBtcPendingSell as setPendingSell, addBtcPnl as addPnl,
} from "../lib/btc-live-db";
...
export const btcPureLagBot = schedules.task({
  id:          "live-bot-btc-purelag-1m",
  cron:        "* * * * *",
  maxDuration: 55,
  ...
  run: async () => {
    ...
    try {
      ...
      if (pos) {
        ...
        // ── Hold phase: check if TP or SL filled ──────────────────────────────
          ...
          if (tpOrder.status === "FILLED") {
            ...
            if (floorQty(btcFree) < 0.00001) {
              const slOrderCheck = await getOrder(SYMBOL, pos.sl_order_id);
              const exitPrice    = parseFloat(slOrderCheck.cummulativeQuoteQty) / parseFloat(slOrderCheck.executedQty);
              const pnl          = (exitPrice - pos.entry_price) * pos.quantity;
              await closePosition(pos.id, { exit_price: exitPrice, pnl, result: "SL" });
              await addPnl(pnl);
              await updateBalance((settings.usdt_balance ?? ALLOCATION) + pnl);
        ...
        if (signal) {
          ...
          log.push({ action: "WATCH", xlmGLRet: spread.toFixed(4), price });
        }
      }
    ...
    await logRun({ actions: log });
    if (hadPosition) {
      console.log("BTC pure-lag bot run:", JSON.stringify(log, null, 2));
      return { ok: true, actions: log };
    }
    ...
    try {
      ...
      if (!pos2) {
        ...
        if (spread2 >= GL_THRESH && glRet2 > 0 && usRet2 < glRet2) {
          ...
          log2.push({ action: "WATCH_30S", xlmGLRet: spread2.toFixed(4), price: price2 });
        }
      }
    } catch (err) {
    ...
    console.log("BTC pure-lag bot run 30s:", JSON.stringify(log2, null, 2));
