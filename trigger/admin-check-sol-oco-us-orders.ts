// One-off manual utility — reports open SOLUSDT orders and the SOL/USDT free balance on
// Binance.US. Runs through Trigger.dev's infrastructure since Binance.US only accepts calls
...
export const adminCheckSolOcoUsOrders = task({
  id: "admin-check-sol-oco-us-orders",
  run: async () => {
    const openOrders = await getOpenOrders("SOLUSDT");
    const usdtFree = await getFreeBalance("USDT");
    const solFree = await getFreeBalance("SOL");
    return { ok: true, openOrders, usdtFree, solFree };
  },
});
